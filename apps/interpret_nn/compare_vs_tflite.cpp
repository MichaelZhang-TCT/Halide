...
int main(int argc, char **argv) {
    ...
    if (use_delegate) {
        delegate_lib = halide_load_library("bin/host/libHalideDelegate.so");
        if (!delegate_lib) {
            std::cerr << "Unable to open Halide Delegate library.\n";
            return 1;
        }
        assert(delegate_lib);
        delegate_factory.create_delegate = (decltype(delegate_factory.create_delegate)) halide_get_library_symbol(delegate_lib, "tflite_plugin_create_delegate");
        assert(delegate_factory.create_delegate);
        delegate_factory.destroy_delegate = (decltype(delegate_factory.destroy_delegate)) halide_get_library_symbol(delegate_lib, "tflite_plugin_destroy_delegate");
